...
@Component({
    selector: 'app-prospectus-request-cmp',
    templateUrl: './prospectus-request.component.html',
    styleUrls: ['./prospectus-request.component.scss']
})

export class ProspectusRequestComponent extends WebformBaseDirective implements OnInit, AfterViewInit, OnDestroy {
    studentInfo: any;
    ...
    ngOnInit() {
        this.getData('signup-form/get-webform-data').then(() => {
            if (!this.isUnderMaintenance) {
                ...
                if (!this.preview) {
                    FormUtils.updateDisplay(this.uniqId, FormType.prospectus_request, this.school.id, this.httpService).catch((err) => {
                        console.log(err);
                    });
                }
    ...
    protected submitData(): Promise<void> {
        const formData = {
            ...
            formType: FormType.prospectus_request,
